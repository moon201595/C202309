...
int main() {
	...
	int delIndex = -1;  // 할 일 삭제를 위한 index 저장 변수, 동작이 안됐을 때를 확인하기 위해 해놓은 것(동작할 때의 값과 전혀 다른 값으로)
	int changeIndex = -1; // 할 일 수정을 위한 index 저장 변수, 동작이 안됐을 때를 확인하기 위해 해놓은 것(동작할 때의 값과 전혀 다른 값으로)
	...
	while (1) {
		int choice = -1; // 사용자 입력 메뉴를 저장하기 위한 변수, 동작이 안됐을 때를 확인하기 위해 해놓은 것(동작할 때의 값과 전혀 다른 값으로)
		...
		case 2:
			...
			scanf_s("%d", &delIndex);
			if (delIndex > taskCount || delIndex <= 0) { // 둘 중 하나만 참이면 참
